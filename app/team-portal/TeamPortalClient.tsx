...
type PortalTeamOption = { id: string; name: string; slug: string; role: string };
...
const workspaces: Array<{ tab: PortalTab; capability: TeamCapability; label: string; description: string }> = [
  ...
  { tab: "stats", capability: "stats_submit", label: "Basketball IQ", description: "Capture complete games, identify training priorities and brief linked players." },
...
export default function TeamPortalClient({ teamId, portalTeams }: { teamId: string; portalTeams: PortalTeamOption[] }) {
  ...
  const visibleTabs = tabs.filter(([key]) => !tabCapabilities[key] || has(tabCapabilities[key]!));
  ...
  return (
    <main className="min-h-screen text-white" style={portalStyle}>
      <header className="relative overflow-hidden border-b border-white/15 bg-[#030b1a]">
        ...
        <div className="relative mx-auto max-w-7xl px-5 py-8 sm:px-6 lg:px-8">
          <div className="flex flex-col gap-6 md:flex-row md:items-end md:justify-between">
            ...
            <div className="flex flex-wrap items-center gap-2">{portalTeams.length > 1 ? <label className="rounded-xl border border-white/10 bg-black/25 px-3 py-2"><span className="mr-2 text-[8px] font-black uppercase text-slate-400">Club</span><select value={teamId} onChange={(event) => window.location.assign(`/team-portal?team_id=${encodeURIComponent(event.target.value)}`)} className="max-w-48 bg-transparent text-xs font-black text-white outline-none">{portalTeams.map((portalTeam) => <option key={portalTeam.id} value={portalTeam.id} className="bg-slate-950">{portalTeam.name} · {TEAM_ROLE_LABELS[portalTeam.role] || portalTeam.role}</option>)}</select></label> : null}<label className="rounded-xl border border-white/10 bg-black/25 px-3 py-2"><span className="mr-2 text-[8px] font-black uppercase text-slate-400">Theme</span><select value={theme} onChange={(event) => changeTheme(event.target.value as PortalTheme)} className="bg-transparent text-xs font-black text-white outline-none"><option value="club" className="bg-slate-950">Club</option><option value="midnight" className="bg-slate-950">Midnight</option><option value="arena" className="bg-slate-950">Arena</option></select></label><Link href={`/teams/${team.slug}`} className="rounded-xl border border-white/10 px-4 py-3 text-xs font-black">Public profile</Link><button onClick={logout} className="rounded-xl border border-red-400/25 bg-red-500/10 px-4 py-3 text-xs font-black text-red-200">Logout</button></div>
          </div>
          <nav aria-label="Club workspaces" className="mt-7"><p className="mb-2 text-[8px] font-black uppercase tracking-[.16em] text-slate-500">Workspace menu</p><div className="flex gap-2 overflow-x-auto pb-1">{visibleTabs.map(([key, label]) => <button key={key} onClick={() => setTab(key)} style={tab === key ? { backgroundColor: themeAccent, color: readableText(themeAccent) } : undefined} className={`shrink-0 rounded-xl px-4 py-2.5 text-[9px] font-black uppercase tracking-[.1em] transition ${tab === key ? "shadow-lg" : "border border-white/10 bg-black/25 text-slate-300 hover:border-white/30 hover:text-white"}`}>{label}</button>)}</div></nav>
        </div>
      </header>

      {tab === "stats" ? <Feature capability="stats_submit" active={has("stats_submit")} title="Basketball IQ">
        <BasketballIQWorkspace teamId={teamId} roster={data.roster} games={data.games} onMessage={setMessage} />
        <div className="mt-6 grid gap-6 lg:grid-cols-[1.05fr_.95fr]">
          <Panel eyebrow="League result" title="Submit final score">
            <p className="mt-3 text-sm leading-6 text-slate-400">The complete player box score and the final league result use separate approval controls. Basketball never records an equal final score; finish overtime first.</p>
            {data.league_memberships.length ? <form onSubmit={(event) => { event.preventDefault(); const form = new FormData(event.currentTarget); void post({ action: "submit_game_result", league_id: form.get("league_id"), opponent_name: form.get("opponent_name"), game_date: form.get("game_date"), venue: form.get("venue"), team_score: form.get("team_score"), opponent_score: form.get("opponent_score"), notes: form.get("notes") }, "stats"); event.currentTarget.reset(); }} className="mt-5 grid gap-3"><select name="league_id" required className={input}><option value="">Choose league</option>{data.league_memberships.map((membership) => <option key={membership.id} value={membership.league_id}>{membership.leagues?.name || "League"} · {membership.division}</option>)}</select><input name="opponent_name" required placeholder="Opponent team" className={input}/><div className="grid grid-cols-2 gap-3"><input name="game_date" required type="datetime-local" className={input}/><input name="venue" placeholder="Venue" className={input}/></div><div className="grid grid-cols-2 gap-3"><input name="team_score" required type="number" min="0" placeholder={`${team.name} score`} className={input}/><input name="opponent_score" required type="number" min="0" placeholder="Opponent score" className={input}/></div><textarea name="notes" rows={3} placeholder="Scorer-sheet or correction note" className={input}/><button disabled={working} className={button}>Send result for verification</button></form> : <div className="mt-5 rounded-xl border border-dashed border-white/15 p-5 text-sm text-slate-500">Super Admin must place this team in a league before league results can be submitted.</div>}
          </Panel>
          <Panel eyebrow="Governance" title={`${data.stat_submissions.length} review records`}>
            <p className="mt-3 text-sm leading-6 text-slate-400">Club data works internally as soon as it autosaves. Public statistics and standings change only after Super Admin approval.</p>
            <div className="mt-5 grid max-h-[28rem] gap-3 overflow-y-auto pr-1">{data.stat_submissions.map((submission) => { const kind = submission.stat_payload?.submission_type; const isResult = kind === "game_result"; const isSession = kind === "team_stat_session"; return <article key={submission.id} className="rounded-xl border border-white/10 bg-black/25 p-4"><div className="flex items-center justify-between gap-3"><p className="font-black">{isResult ? `vs ${submission.stat_payload?.opponent_name || "Opponent"}` : isSession ? `Complete box score · ${submission.stat_payload?.player_rows || 0} players` : submission.stat_payload?.player_name || "Stat submission"}</p><Status ready={submission.status === "approved"} pending={submission.status === "pending"}/></div><p className="mt-2 text-xs text-slate-500">{isResult ? `${submission.stat_payload?.league_name || "League"} · ${submission.stat_payload?.team_score}–${submission.stat_payload?.opponent_score}` : isSession ? `${submission.stat_payload?.mode || "Team"} capture · ${submission.stat_payload?.linked_official_players || 0} official player links` : `PTS ${submission.stat_payload?.points ?? "—"} · REB ${submission.stat_payload?.rebounds ?? "—"} · AST ${submission.stat_payload?.assists ?? "—"}`}</p></article>; })}{!data.stat_submissions.length ? <Empty text="No result or stat submissions yet."/> : null}</div>
            <div className="mt-5 grid gap-2">{data.leaderboard_links.map((link) => <Link key={link.href} href={link.href} className="rounded-xl border border-white/10 bg-black/25 p-4 text-sm font-black text-[var(--club-accent)]">{link.title} →</Link>)}</div>
          </Panel>
        </div>
      </Feature> : null}
      ...
      {tab === "command" ? <section className="mx-auto max-w-7xl px-5 py-8 sm:px-6 lg:px-8">
        <div className="grid gap-3 sm:grid-cols-2 lg:grid-cols-5">{has("roster_manage") ? <Metric label="Roster" value={data.roster.length} /> : null}{has("training_manage") ? <Metric label="Training" value={data.training.length} /> : null}{has("stats_submit") ? <Metric label="Games" value={data.games.length} /> : null}{has("media_submit") ? <Metric label="Media gaps" value={data.media_summary.missing_posters + data.media_summary.missing_full_games} tone="orange" /> : null}{has("stats_submit") || has("media_submit") ? <Metric label="In review" value={(has("media_submit") ? data.media_summary.pending : 0) + (has("stats_submit") ? data.stat_submissions.filter((item) => item.status === "pending").length : 0)} /> : null}</div>
        <div className="mt-6"><Panel eyebrow={`${roleLabel} access`} title="Choose a workspace"><p className="mt-3 max-w-3xl text-sm leading-6 text-slate-400">{roleDescription} Select a workspace below to start working.</p><div className="mt-5 grid gap-3 sm:grid-cols-2 lg:grid-cols-3">{workspaces.filter((workspace) => has(workspace.capability)).map((workspace) => <WorkspaceCard key={`${workspace.tab}-${workspace.capability}`} workspace={workspace} onOpen={() => setTab(workspace.tab)} />)}</div></Panel></div>
        <div className="mt-6 grid gap-6 lg:grid-cols-[1.25fr_.75fr]">
          {has("media_submit") ? <Panel eyebrow="Next best actions" title="What needs attention">
          ...
          {has("stats_submit") ? <Panel eyebrow="Competition intelligence" title="Leaderboards">
...
function WorkspaceCard({ workspace, onOpen }: { workspace: (typeof workspaces)[number]; onOpen: () => void }) {
  return <button type="button" onClick={onOpen} className="rounded-xl border border-emerald-400/20 bg-emerald-500/[.07] p-4 text-left transition hover:-translate-y-0.5 hover:border-[var(--club-accent)] hover:bg-white/[.06]"><span className="flex items-start justify-between gap-3"><span className="text-base font-black uppercase">{workspace.label}</span><span className="rounded-full bg-emerald-500/15 px-2 py-1 text-[7px] font-black uppercase text-emerald-300">Open</span></span><span className="mt-2 block text-xs leading-5 text-slate-400">{workspace.description}</span><span className="mt-4 block text-[8px] font-black uppercase tracking-[.1em] text-[var(--club-accent)]">Enter workspace →</span></button>;
}
function Feature({ active, children, compact = false }: { capability: TeamCapability; active: boolean; title: string; children: React.ReactNode; compact?: boolean }) { if (!active) return null; return <section className={compact ? "" : "mx-auto max-w-7xl px-5 py-8 sm:px-6 lg:px-8"}>{children}</section>; }
