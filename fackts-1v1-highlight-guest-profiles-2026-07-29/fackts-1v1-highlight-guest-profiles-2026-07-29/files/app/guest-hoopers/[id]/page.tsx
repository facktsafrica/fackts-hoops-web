...
function belongsToGuest(row: OneOnOneRow, guest: GuestHooper) {
  return (
    row.guest_hooper_id === guest.id ||
    row.opponent_guest_hooper_id === guest.id ||
    Boolean(
      guest.source_player_id &&
        (row.fackts_player_id === guest.source_player_id ||
          row.opponent_player_id === guest.source_player_id)
    )
  );
}
...
async function getProfileData(id: string): Promise<ProfileData | null> {
  ...
  if (guestResult.error || !guestResult.data) return null;

  const guest = guestResult.data as GuestHooper;

  const [
    guestStatsResult,
    formerPlayerStatsResult,
    oneOnOneResult,
    playersResult,
    guestsResult,
  ] = await Promise.all([
    supabase
      ...
      .select("*")
      .eq("guest_hooper_id", guest.id)
      .order("created_at", { ascending: false }),
    guest.source_player_id
      ? supabase
          .from("player_game_stats")
          .select("*")
          .eq("player_id", guest.source_player_id)
          .order("created_at", { ascending: false })
    ...
    supabase.from("players").select("id, full_name, name, nickname"),
    supabase.from("guest_hoopers").select("id, full_name, name, nickname"),
  ]);
  ...
  ((playersResult.data || []) as Person[]).forEach((player) => {
    playerMap.set(player.id, player);
  });

  const guestMap = new Map<string, Person>();
  ((guestsResult.data || []) as Person[]).forEach((person) => {
...
export default async function GuestHooperProfilePage({
  ...
  const role =
    ...
    (guest.guest_type === "external_player"
      ? "External Player"
      : "Guest Hooper");
  const about = guest.bio || guest.notes || "";
